...
import io.angularpay.invoice.exceptions.ValidationException;
...
import static io.angularpay.invoice.helpers.Helper.*;
...
public abstract class AbstractCommand<T extends AccessControl, R> {
    ...
    public R execute(T request) {
        try {
            ...
            if (this instanceof UserNotificationsPublisherCommand && response instanceof InvoiceRequestSupplier) {
                log.info("publishing {} User Notification to REDIS", this.name);
                Executors.newSingleThreadExecutor().submit(() -> {
                    ((UserNotificationsPublisherCommand)this).publishUserNotification((InvoiceRequestSupplier)response);
                });
            }
