...
import static io.angularpay.invoice.exceptions.ErrorCode.REQUEST_CANCELLED_ERROR;
...
import static io.angularpay.invoice.helpers.Helper.getAllParties;
import static io.angularpay.invoice.models.UserNotificationType.*;
...
public class UpdateRequestStatusCommand extends AbstractCommand<UpdateRequestStatusCommandRequest, GenericCommandResponse>
    ...
    private GenericCommandResponse updateRequestStatus(UpdateRequestStatusCommandRequest request) throws OptimisticLockingFailureException {
        InvoiceRequest found = getRequestByReferenceOrThrow(this.mongoAdapter, request.getRequestReference());
        return this.commandHelper.updateProperty(found, request::getStatus, found::setStatus);
    }
    ...
    public UserNotificationType getUserNotificationType(GenericCommandResponse commandResponse) {
        RequestStatus status = commandResponse.getInvoiceRequest().getStatus();
        switch (status) {
            case ACTIVE:
    ...
    public List<String> getAudience(GenericCommandResponse commandResponse) {
        return getAllParties(commandResponse.getInvoiceRequest());
    }

    @Override
    public String convertToUserNotificationsMessage(UserNotificationBuilderParameters<GenericCommandResponse, InvoiceRequest> parameters) throws JsonProcessingException {
        ...
        String summary;
        if (parameters.getUserReference().equalsIgnoreCase(parameters.getRequest().getMerchant().getUserReference())) {
            summary = "your Invoice Request was marked as: " + status;
        } else {
            summary = "an Invoice Request assigned to you, was marked as: " + status;
        }
